...
export type elseClause = { tag: "elseClause", then: CExp[]; }
...
export const makeElseClause = (then: CExp[]) : elseClause =>
    ({ tag: "elseClause", then: then });
...
const parseCondClauses = (params: Sexp[]): Result<CondClause> =>
  isEmpty(params)
    ? makeFailure(`Empty cond clauses.`)
    : params.length !== 2
    ? makeFailure(`Invalid cond clause: ${JSON.stringify(params, null, 2)}`)
    : first(params) === "else"
